    ...
    <div className="space-y-8">
      ...
      <div className="relative max-w-md">
        ...
        <Input
          ...
          className="pl-10 h-10 rounded-full bg-card shadow-sm border-border/50"
          aria-label="搜索应用"
        />
      </div>
      ...
      {loading ? (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3].map((i) => (
            <Card key={i} className="h-48 animate-pulse bg-muted/20 rounded-3xl" />
          ))}
        </div>
      ) : showTabs ? (
    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {apps.map((app) => (
        <Card
          ...
          className="group relative overflow-hidden rounded-2xl border-border/50 bg-card/50 transition-all hover:shadow-2xl hover:-translate-y-1"
        >
          ...
          <CardFooter className="bg-muted/30 pt-4 flex justify-between items-center px-6">
            <span className="text-xs font-bold text-muted-foreground">
              {app.author || app.owner_name || app.slug}
            </span>
